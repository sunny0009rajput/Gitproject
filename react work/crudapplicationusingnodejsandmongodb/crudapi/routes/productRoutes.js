...
router.post('/products',auth,requireAdmin,upload.fields([
        ...
        const products = new ProductModel(
            {
                ...
                mainPhoto: req.files['mainPhoto'] ? { contentType: req.files['mainPhoto'][0].mimetype, data: req.files['mainPhoto'][0].buffer } : null,
                sub1Photo: req.files['sub1Photo'] ? { contentType: req.files['sub1Photo'][0].mimetype, data: req.files['sub1Photo'][0].buffer } : null,
                sub2Photo: req.files['sub2Photo'] ? { contentType: req.files['sub2Photo'][0].mimetype, data: req.files['sub2Photo'][0].buffer } : null,
                sub3Photo: req.files['sub3Photo'] ? { contentType: req.files['sub3Photo'][0].mimetype, data: req.files['sub3Photo'][0].buffer } : null,
...
router.get('/products/:id', async (req, res) => {
    try {
        ...
        const updatedProduct = {
            ...products._doc,
            mainPhoto: products.mainPhoto ? `data:${products.mainPhoto.contentType};base64,${products.mainPhoto.data.toString('base64')}` : null,
            sub1Photo: products.sub1Photo ? `data:${products.sub1Photo.contentType};base64,${products.sub1Photo.data.toString('base64')}` : null,
            sub2Photo: products.sub2Photo ? `data:${products.sub2Photo.contentType};base64,${products.sub2Photo.data.toString('base64')}` : null,
            sub3Photo: products.sub3Photo ? `data:${products.sub3Photo.contentType};base64,${products.sub3Photo.data.toString('base64')}` : null,
            product_date: products.product_date,
...
router.put('/products/:id',auth,requireAdmin,upload.fields([
    { name: "mainPhoto", maxCount: 1 },
    { name: "sub1Photo", maxCount: 1 },
    { name: "sub2Photo", maxCount: 1 },
    { name: "sub3Photo", maxCount: 1 },
    ...
    try {
      ...
      if (req.files["sub3Photo"]) {
        ...
        const products = await ProductModel.findByIdAndUpdate(req.params.id, updateData, { new: true, runValidators: true });
