...
const LeftSidebar = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      {/* Left Sidebar */}
      <div className="bg-slate-800 text-white transition-all duration-300 ease-in-out
                      w-16 lg:w-64 flex flex-col shadow-xl">
        ...
        <div className="flex items-center justify-center lg:justify-start p-4 border-b border-slate-700">
          ...
          <span className="hidden lg:block ml-3 text-xl font-bold">Brand</span>
        </div>
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                ...
                <span className="hidden lg:block ml-3 font-medium">{link.name}</span>
                
                {/* Tooltip for mobile/collapsed view */}
                <div className="absolute left-full ml-2 px-2 py-1 bg-gray-900 text-white text-sm rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 whitespace-nowrap lg:hidden">
                  {link.name}
        ...
        <div className="border-t border-slate-700 p-2">
          <div className={`flex items-center px-3 py-3 rounded-lg cursor-pointer transition-all duration-200 hover:bg-slate-700 group ${
            ...
            <LogOut className="hidden lg:block w-4 h-4 text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
            
            {/* Tooltip for profile on mobile */}
            <div className="absolute left-full ml-2 px-2 py-1 bg-gray-900 text-white text-sm rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 whitespace-nowrap lg:hidden">
              John Doe
              <div className="absolute top-1/2 left-0 transform -translate-y-1/2 -translate-x-1 w-2 h-2 bg-gray-900 rotate-45"></div>
            </div>
