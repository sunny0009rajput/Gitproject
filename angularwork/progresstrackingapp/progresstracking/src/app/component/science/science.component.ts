...
export class ScienceComponent{
  ...
  toggleSubtasks(task: Task) {
    ...
    if (this.isBrowser()) {
      localStorage.setItem('sciencetasks', JSON.stringify(this.tasks));
    }
  }

  // completedSubtasks():number {
  //   return this.tasks.filter(task => task.subtasks.every(subtask => subtask.completed)).length;
  // }
  ...
  loadTasksFromLocalStorage() {
    const savedTasks = localStorage.getItem('sciencetasks');
    if (savedTasks) {
