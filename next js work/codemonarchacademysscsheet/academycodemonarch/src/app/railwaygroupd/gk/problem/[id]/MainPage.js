...
import gkData from "../../../../data/groupdgk.json";
...
const handleTouchEnd = () => {
  ...
  const handleNextQuestion = () => {
    if (currentQuestionIndex < allQuestions.length - 1) {
      ...
      router.push(
        `/railwaygroupd/gk/problem/${allQuestions[nextIndex].id}?chapter=${chapter}`
      );
    }
  };
  ...
  const handlePreviousQuestion = () => {
    if (currentQuestionIndex > 0) {
      ...
      setShowSolution(false);
      router.push(
        `/railwaygroupd/gk/problem/${allQuestions[prevIndex].id}?chapter=${chapter}`
      );
    }
  };
  ...
  return (
    ...
    <div
      className="min-h-screen bg-black text-white px-4 sm:px-8 md:px-16 pt-30"
