...
import englishData from "../../../../data/english.json"; // 👈 import your local data
...
export default function ProblemPage() {
  ...
  // ✅ Fetch data from local file instead of API
   // ✅ Add these 4 state hooks at the top:
...
const [touchEndY, setTouchEndY] = useState(0);
  ...
  // ✅ Swipe Handlers (Improved & Scroll-safe)
...
const handleTouchEnd = () => {
  ...
  // 🚫 Ignore small moves (less than 50px horizontally or if mostly vertical)
  ...
  const handleNextQuestion = () => {
    if (currentQuestionIndex < allQuestions.length - 1) {
      ...
      router.push(
        `/sscsheet/english/problem/${allQuestions[nextIndex].id}?chapter=${chapter}`
      );
    }
  };
  ...
  const handlePreviousQuestion = () => {
    if (currentQuestionIndex > 0) {
      ...
      setShowSolution(false);
      router.push(
        `/sscsheet/english/problem/${allQuestions[prevIndex].id}?chapter=${chapter}`
      );
    }
  };
  ...
  return (
    <>
        <Navbar/>
    <div className="min-h-screen bg-black text-white px-4 sm:px-8 md:px-16 pt-30"
