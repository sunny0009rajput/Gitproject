    ...
    const loadData = async () => {
      try {
        ...
        chaptersList.forEach((chapter) => {
          const data = englishData[chapter.title] || englishData[chapter.title.replace(/\s/g, "_")];
                          ...
                          {favouriteProblems.map((q) => {
                            ...
                            return (
                              <tr key={key} className="border-t border-zinc-800 hover:bg-zinc-800 transition">
                                <td className="px-4 py-4">{q.question?.type === "text" ? trimText(q.question.content, 80) : <Image src={q.question.content} alt="question" width={300} height={200} className="max-w-full rounded" />}</td>
                                <td className="px-4 py-4">
                                  <FileText onClick={() =>
                                window.open(`/sscsheet/english/problem/${q.id}?chapter=${encodeURIComponent(chapter.title)}`, "_blank")
