...
export async function generateMetadata() {
  ...
  const title = "SSC English Questions & Answers – CodeMonarch Academy";
  const description = `Access SSC English chapters like ${chapters
    .map((c) => c.title)
    .join(", ")} with grammar, comprehension, vocabulary, and previous year questions with explanations.`;
...
function SEOStaticContent() {
  ...
  const chapterDetails = chapters.map((ch) => {
    const questions = englishData[ch.title]?.[1]?.Questions || [];
  ...
  return (
    <div id="seo-content" style={{ display: "none" }}>
      <h1>SSC English Questions and Answers – CodeMonarch Academy</h1>
