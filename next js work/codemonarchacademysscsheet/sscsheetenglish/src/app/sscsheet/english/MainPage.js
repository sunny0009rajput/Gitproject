...
import englishData from "../../data/english.json"; // local JSON
import {
  getChapterProgress,
  updateProblemStatus,
  updateFavourite,
...
export default function Page() {
  ...
  useEffect(() => {
    const fetchChapters = async () => {
      const chaptersList = englishData?.chapters || [];
  ...
  const loadChapterData = (chapter) => {
    ...
    const data = englishData[chapterTitle] || englishData[chapterKey];
  ...
  const loadChapterProgress = async (chapter) => {
    const progress = await getChapterProgress("English", chapter.title);
    const completedMap = Object.fromEntries(
      progress.completedProblems.map((id) => [
        `${chapter.title}-problem-${id}`,
        true,
      ])
    );
  ...
  const handleProblemCheck = async (chapterTitle, problemId) => {
    ...
    await updateProblemStatus("English", chapterTitle, problemId, completed);
  };
  ...
  const handleFavourite = async (chapterTitle, problemId) => {
    ...
    await updateFavourite("English", chapterTitle, problemId, isFav);
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-18 px-2 py-2 lg:px-28 lg:py-28">
      <Navbar />
      <h1 className="text-3xl text-center font-bold pt-10 mb-6">
        SSC English Chapters
      </h1>
      <p className="text-xl text-center font-bold pt-2 mb-4">
        This course is designed for aspirants who want to master General
        Knowledge<br></br> for SSC exams in a structured and easy-to-understand
        way.<br></br>Each chapter is carefully organized to cover concepts,
        important facts,<br></br> and practice questions, helping students build
        confidence and excel in exams.
      </p>
      {/* Revision button */}
      <div className="flex justify-end mb-4">
        <button
          onClick={() => router.push("/sscsheet/english/revision")}
      ...
      {chapters.map((chapter) => {
        ...
        const progressPercent =
          totalProblems > 0 ? (completedCount / totalProblems) * 100 : 0;
        ...
        return (
          ...
          >
            ...
            {isExpanded && totalProblems > 0 && (
              <div className="border-t border-zinc-800 p-4 sm:p-6 overflow-x-auto">
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-4 text-center">
                            <button
                              onClick={() =>
                                handleFavourite(chapter.title, q.id)
                              }
