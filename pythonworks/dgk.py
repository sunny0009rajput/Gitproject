...
def extract_english_pdf_to_json(pdf_path, output_json="gs.json"):
    ...
    # Step 2: Split by sections (RRC / SSC / etc.)
    section_pattern = re.compile(
        r"(?=((?:SSC|RRC|RRB)[A-Za-z0-9 ]+\d{4}))",
        re.IGNORECASE
    )
    ...
    for i in range(1, len(sections), 2):
        ...
        for q_num, q_text in question_blocks:
            ...
            # 🔥 NEW STATUS FORMAT
            # Example: RRC Group D 19/09/2022 (Afternoon)
            ...
            # Remove status from question
            if status_match:
                q_text = q_text.replace(status_match.group(1), "")
...
# Example
extract_english_pdf_to_json("C:/Users/sunny/Downloads/generalsciencetext.pdf")
